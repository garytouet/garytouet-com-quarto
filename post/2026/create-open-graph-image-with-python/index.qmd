...
For most of this project, we’ll use the [_pillow_ library](https://pypi.org/project/pillow/). To work with `.svg` graphical elements (not natively supported by pillow), we’ll also use _cairosvg_.
...
Let's start with the simplest text elements. The _domain name followed by an icon_ and _mix of emojis and text_ will be addressed later.
...
4. The drawing command places the text at specified coordinates, assigns a font and color, and sets the `language` to English using language. Some fonts include language-specific settings.
...
2. The calculation is performed here.
...
Here, _cairosvg_ is used: the `svg2png` function resizes the `.svg` icon, converts it to `.png`, and places the result on the canvas.
